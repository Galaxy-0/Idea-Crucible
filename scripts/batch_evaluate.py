...
def evaluate_one(idea_path: Path, rules_dir: Path, model_cfg: Path) -> Path:
    ...
    with open(out_json, "w", encoding="utf-8") as f:
        payload = verdict.model_dump() if hasattr(verdict, "model_dump") else verdict.__dict__
...
def collect_stats(verdict_paths: List[Path]) -> Dict[str, Dict[str, float]]:
    ...
    redline_hit_rate = {k: (v / total if total else 0.0) for k, v in redline_counts.items()}
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Batch evaluate ideas and compute simple stats.")
    ...
    ap.add_argument("--pattern", type=str, default="*.yaml", help="Glob pattern under ideas-dir")
    ap.add_argument("--stats", action="store_true", help="Compute and write stats JSON to reports/_stats.json")
    args = ap.parse_args()
